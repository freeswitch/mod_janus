...
switch_status_t serversAdd(switch_xml_t xmlint)
{
	...
	for (param = switch_xml_child(xmlint, "param"); param; param = param->next) {
		...
		if (!strcmp(pVarStr, "url") && !zstr(pValStr)) {
...
#ifndef HAVE_MOD_JANUS_WS
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
			"mod_janus: server=%s url uses WebSockets (ws:// or wss://) but this build was compiled without libks WebSocket support. "
			"Install libks or libks2, rebuild FreeSWITCH with libks enabled so mod_janus can link it, or change the url to http:// or https://.\n",
			pName);
		return SWITCH_STATUS_FALSE;
#else
